    ...
    for i in "${!PLAYLISTITEMS[@]}"; do
        ...
        if ! [ -f "$OUTPUT/$afile" ]; then
            if ! [ -f "$dfile" ] || ! [ -f "./inprog/$afile" ]; then
                yt-dlp -f "m4a/bestaudio/best" "https://www.youtube.com/watch?v=$id" --no-warnings -q --progress \
                    ...
                    -x --audio-quality 0 --audio-format m4a || true
            fi
            if ! [ -f "$dfile" ]; then
                echo "https://youtu.be/$id, DLFAIL" >> "./inprog/log"
                printf "\r\033[KFailed to download %s\n" "$afile"
                continue
            fi
            ...
            if [[ $description == *"Auto-generated by YouTube." ]]; then
                ...
                if (( ${#res[@]} != 0 )); then # if not empty (i.e. parse success)
                    ...
                    echo "https://youtu.be/$id, REGEXFAIL" >> "./inprog/log"
                    printf "\r\033[KFailed to extract info from %s\n" "$dfile"
                fi
            else
                echo "https://youtu.be/$id, DESCFORMAT" >> "./inprog/log"
